...
int main(int argc, char* argv[]) {
    if (argc != 2) {
        printf("Использование: %s <размер стека в байтах>\n", argv[0]);
        return 1;
    }

    size_t stack_size = atoi(argv[1]); // Размер стека
    ...
    printf("Сервер запущен на порту %d с размером стека %zu байт...\n", PORT, stack_size);
    ...
    while (1) {
        ...
        *client_socket_ptr = client_socket;
        ...
        if (pthread_create(&thread_id, &attr, handle_request, client_socket_ptr) != 0) {
            perror("Ошибка при создании потока");
            free(client_socket_ptr);
            close(client_socket);
            break; // Прерываем цикл при ошибке создания потока
        }
